...
const AidLocation = require('../models/aidLocation.model');

// Create a new fixed aid distribution location
router.post('/', async (req, res) => {
  try {
    const newLocation = new AidLocation(req.body);
...
router.get('/', async (req, res) => {
  try {
    const locations = await AidLocation.find();
    res.json(locations);
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    const location = await AidLocation.findById(req.params.id);
...
router.put('/:id', async (req, res) => {
  try {
    const updatedLocation = await AidLocation.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedLocation = await AidLocation.findByIdAndDelete(req.params.id);
