...
const orphanSupport = require('../models/orphanSupport.model');

// Create a new fixed aid distribution location
router.post('/', async (req, res) => {
  try {
    const newLocation = new orphanSupport(req.body);
...
router.get('/', async (req, res) => {
  try {
    const locations = await orphanSupport.find();
    res.json(locations);
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    const location = await orphanSupport.findById(req.params.id);
...
router.put('/:id', async (req, res) => {
  try {
    const updatedLocation = await orphanSupport.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedLocation = await orphanSupport.findByIdAndDelete(req.params.id);
